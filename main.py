...
if __name__ == "__main__":
    ...
    """
    if check_arguments(sys.argv[1:]):
        if check_date(sys.argv[1]):
            converter = CurrencyConverter(sys.argv[2].upper(), sys.argv[3].upper(), date=sys.argv[1])
            if converter.check_currencies():
                amt = float(input("Enter the amount to be converted\t"))
                amount = converter.get_historical_rate(amt = amt)
                print(amount)
